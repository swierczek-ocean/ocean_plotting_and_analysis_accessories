...
volume_Bot3 = zeros(nt,1);

for t=1:12
    
    charstate = [str,'diag_state.',num2str(1460*(t+1),'%010.f')];
    vel = rdmds(charstate,'rec',3:5);
    vel = vel([x x(end)+1],[y y(end)+1],[z z(end)+1],:);
    
    volume_N3(t) = sum(sum(vel(1:(end-1),end,1:(end-1),2).*areaSouth(:,end,:)));
    volume_S3(t) = sum(sum(vel(1:(end-1),1,1:(end-1),2).*areaSouth(:,1,:)));
    
    volume_E3(t) = sum(sum(vel(end,1:(end-1),1:(end-1),1).*areaWest(end,:,:)));
    volume_W3(t) = sum(sum(vel(1,1:(end-1),1:(end-1),1).*areaWest(1,:,:)));
    
    volume_Bot3(t) = sum(sum(vel(1:(end-1),1:(end-1),end,3).*areaTop(:,:,end)));
    clear vel
end % for t
%% volume
...
heat_Bot3 = zeros(nt,1);


for t=1:12

    charheat = [strb,'diag_T_budget.',num2str(1460*(t+1),'%010.f')];
    ...
    heat_E3(t) = cp_oce*rho*sum(sum(adv(end,1:(end-1),1:(end-1),1)));
    heat_W3(t) = cp_oce*rho*sum(sum(adv(1,1:(end-1),1:(end-1),1)));
    
    heat_Bot3(t) = cp_oce*rho*sum(sum(adv(1:(end-1),1:(end-1),end,3)));
    clear adv diff
...
salt_Bot3 = zeros(nt,1);

for t=1:12
    
    charsalt = [strb,'diag_S_budget.',num2str(1460*(t+1),'%010.f')];
    ...
    salt_E3(t) = rho*sum(sum(adv(end,1:(end-1),1:(end-1),1)))/1000;
    salt_W3(t) = rho*sum(sum(adv(1,1:(end-1),1:(end-1),1)))/1000;
    
    salt_Bot3(t) = rho*sum(sum(adv(1:(end-1),1:(end-1),end,3)))/1000;
    clear adv diff
    
    charsaltflux = [str,'diag_airsea.',num2str(1460*(t+1),'%010.f')];
...
carbon_Bot3 = zeros(nt,1);

for t=1:12
    
    charcarbon = [strb,'diag_dic_budget.',num2str(1460*(t+1),'%010.f')];
    ...
    carbon_N3(t) = gty*12.0107*sum(sum(adv(1:(end-1),end,:,2)))/1000;
    carbon_S3(t) = gty*12.0107*sum(sum(adv(1:(end-1),1,:,2)))/1000;
    
    carbon_E3(t) = gty*12.0107*sum(sum(adv(end,1:(end-1),1:(end-1),1)))/1000;
    carbon_W3(t) = gty*12.0107*sum(sum(adv(1,1:(end-1),1:(end-1),1)))/1000;
    
    carbon_Bot3(t) = gty*12.0107*sum(sum(adv(1:(end-1),1:(end-1),end,3)))/1000;
    clear adv diff
    ...
    carbon_surf3(t) = gty*12.0107*sum(sum(cflux.*area))/1000;
end % for t
%% carbon
...
oxygen_Bot3 = zeros(nt,1);

for t=1:12
    
    charoxygen = [strb,'diag_o2_budget.',num2str(1460*(t+1),'%010.f')];
    ...
    oxygen_N3(t) = gty*15.9994*sum(sum(adv(1:(end-1),end,:,2)))/1000;
    oxygen_S3(t) = gty*15.9994*sum(sum(adv(1:(end-1),1,:,2)))/1000;
    
    oxygen_E3(t) = gty*15.9994*sum(sum(adv(end,1:(end-1),1:(end-1),1)))/1000;
    oxygen_W3(t) = gty*15.9994*sum(sum(adv(1,1:(end-1),1:(end-1),1)))/1000;
    
    oxygen_Bot3(t) = gty*15.9994*sum(sum(adv(1:(end-1),1:(end-1),end,3)))/1000;
    clear adv diff
    
    charoxygenflux = [str,'diag_airsea.',num2str(1460*(t+1),'%010.f')];
    o2flux = rdmds(charoxygenflux,'rec',4);
    o2flux = o2flux(x,y);
    
    oxygen_surf3(t) = gty*15.9994*sum(sum(o2flux.*area))/1000;
end % for t
%% oxygen
...
nitrate_Bot3 = zeros(nt,1);

for t=1:12
    
    charnitrate = [strb,'diag_no3_budget.',num2str(1460*(t+1),'%010.f')];
    ...
    nitrate_N3(t) = gty*14.0067*sum(sum(adv(1:(end-1),end,:,2)))/1000;
    nitrate_S3(t) = gty*14.0067*sum(sum(adv(1:(end-1),1,:,2)))/1000;
    
    nitrate_E3(t) = gty*14.0067*sum(sum(adv(end,1:(end-1),1:(end-1),1)))/1000;
    nitrate_W3(t) = gty*14.0067*sum(sum(adv(1,1:(end-1),1:(end-1),1)))/1000;
    
    nitrate_Bot3(t) = gty*14.0067*sum(sum(adv(1:(end-1),1:(end-1),end,3)))/1000;
    clear adv diff
