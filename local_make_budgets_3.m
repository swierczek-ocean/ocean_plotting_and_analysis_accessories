...
str = '../2019_12_December_analysis/Output/3/';
...
% for t=2:13
%     
%     charno3 = [str,'diag_no3_budget.',num2str(1460*t,'%010.f')];
%     % % tendency due to dilution
%     % % diagnostic: FORCTR04
...
%     % total tendency
%     
%     snap = rdmds([str,'diag_snaps'],[1460*(t-1) 1460*t],'rec',6);
...
for t=2:13
    ...
    fprintf('sum of net shortwave heating: %g \n',sum(sum(oceQsw)))
    % tendency due to air-sea flux
    % diagnostic: TFLUX
    ...
    fprintf('sum of total surface heat flux: %g \n',sum(sum(sum(flux))))
    % advection
    ...
    fprintf('sum of total advective heat flux: %g \n',sum(sum(sum(advflux))))
    ...
    fprintf('sum of total heat mixing: %g \n',sum(sum(sum(sum(diffflux)))))
    % total tendency
    
    snap = rdmds([str,'diag_snaps'],[1460*(t-1) 1460*t],'rec',1);
    heattend_calc(:,:,:,t-1) = diff(snap(x,y,z,:),1,4)/dt;
    
    fprintf('sum of total calculated heat tendency: %g \n',sum(sum(sum(sum(heattend_calc(:,:,:,t-1))))))
    tmp = rdmds(charheat,'rec',10);
    heattend(:,:,:,t-1) = tmp(x,y,z)./86400;
    fprintf('sum of total heat tendency: %g \n',sum(sum(sum(sum(heattend(:,:,:,t-1))))))
...
for t=2:13
    ...
    fprintf('sum of total divergent salt flux: %g \n',sum(sum(sum(sum(saltdiv(:,:,:,t-1))))))
...
%     res_intz = squeeze(cumsum(saltres.*dzt,3))*speryr; %clear res
%     
%     
...
% calculate tendencies in mol/m3/s
for t=2:13
    charo2 = [str,'diag_o2_budget.',num2str(1460*t,'%010.f')];
    % tendency due to air-sea flux
    % diagnostic: BLGOFLX
    % surface flux (mol/m3/s)
    charairsea = [str,'diag_airsea.',num2str(1460*t,'%010.f')];
    ...
    snap = rdmds([str,'diag_snaps'],[1460*(t-1) 1460*t],'rec',5);
