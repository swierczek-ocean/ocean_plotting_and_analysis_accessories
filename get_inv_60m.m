...
Heat_60m_TS_3 = zeros(192,132,365);
Heat_60m_TS_6 = zeros(384,260,365);
Heat_60m_TS_12 = zeros(756,512,365);

Carbon_60m_TS_3 = zeros(192,132,365);
Carbon_60m_TS_6 = zeros(384,260,365);
Carbon_60m_TS_12 = zeros(756,512,365);

for ii=1:365
    charstate = [strs3,'diag_state.',num2str(48*(ii+31),'%010.f')];
    temp = rdmds(charstate,'rec',1);
    Heat_60m_TS_3(:,:,ii) = sum(temp(:,:,1:8).*volume3,3)./depth3;
    clear temp
    
    charstate = [strs6,'diag_state.',num2str(72*(ii+31),'%010.f')];
    temp = rdmds(charstate,'rec',1);
    Heat_60m_TS_6(:,:,ii) = sum(temp(:,:,1:8).*volume6,3)./depth6;
    clear temp
    
    charstate = [strs12,'diag_state.',num2str(360*(ii+31),'%010.f')];
    temp = rdmds(charstate,'rec',1);
    Heat_60m_TS_12(:,:,ii) = sum(temp(:,:,1:16).*volume12,3)./depth12;
    clear temp
    
    charbgc = [strs3,'diag_bgc.',num2str(48*(ii+31),'%010.f')];
    temp = rdmds(charbgc,'rec',1);
    Carbon_60m_TS_3(:,:,ii) = sum(temp(:,:,1:8).*volume3,3)./depth3;
    clear temp
    
    charbgc = [strs6,'diag_bgc.',num2str(72*(ii+31),'%010.f')];
    temp = rdmds(charbgc,'rec',1);
    Carbon_60m_TS_6(:,:,ii) = sum(temp(:,:,1:8).*volume6,3)./depth6;
    clear temp
    
    charbgc = [strs12,'diag_bgc.',num2str(360*(ii+31),'%010.f')];
    temp = rdmds(charbgc,'rec',1);
    Carbon_60m_TS_12(:,:,ii) = sum(temp(:,:,1:16).*volume12,3)./depth12;
    clear temp
end
...
for ii=1:192
    for jj=1:132
        if (hFacC3(ii,jj,1)==0)
            Heat_60m_TS_3(ii,jj) = NaN;
            Carbon_60m_TS_3(ii,jj) = NaN;
        end
    end
end
...
for ii=1:384
    for jj=1:260
        if (hFacC6(ii,jj,1)==0)
            Heat_60m_TS_6(ii,jj) = NaN;
            Carbon_60m_TS_6(ii,jj) = NaN;
        end
    end
end
...
for ii=1:756
    for jj=1:512
        if (hFacC12(ii,jj,1)==0)
            Heat_60m_TS_12(ii,jj) = NaN;
            Carbon_60m_TS_12(ii,jj) = NaN;
        end
    end
end

Heat_60m_mean_3 = mean(Heat_60m_TS_3,3);
Heat_60m_std_3 = std(Heat_60m_TS_3,0,3);
Heat_60m_mean_6 = mean(Heat_60m_TS_6,3);
Heat_60m_std_6 = std(Heat_60m_TS_6,0,3);
Heat_60m_mean_12 = mean(Heat_60m_TS_12,3);
Heat_60m_std_12 = std(Heat_60m_TS_12,0,3);

clear Heat_60m_TS*
save Heat_60m_mean_std Heat_60m_mean* Heat_60m_std*
clear Heat*

Carbon_60m_mean_3 = mean(Carbon_60m_TS_3,3);
Carbon_60m_std_3 = std(Carbon_60m_TS_3,0,3);
Carbon_60m_mean_6 = mean(Carbon_60m_TS_6,3);
Carbon_60m_std_6 = std(Carbon_60m_TS_6,0,3);
Carbon_60m_mean_12 = mean(Carbon_60m_TS_12,3);
Carbon_60m_std_12 = std(Carbon_60m_TS_12,0,3);

clear Carbon_60m_TS*
save Carbon_60m_mean_std Carbon_60m_mean* Carbon_60m_std*
clear Carbon*
