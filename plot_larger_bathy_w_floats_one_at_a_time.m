...
%         16,17,3,1,5,22,83,81,78,76,73,71,31,44,6,5,19];
%     cind = randperm(length(colors),num_floats);
%     color_array(kk,:) = cind;
    floats_color = zeros(35,3);
    floats_color(kk,:) = [1,1,1];
    fsz = 65;
    ...
    ytickformat('degrees')
    xtickformat('degrees')
    title('Argentine Basin and surrounding area with SOCCOM float profiles','FontWeight','Normal','FontSize',18)
    axis(coords)
    acc_movie
    acc_plots
    hold off
    print('-r200',['FLOATS/AB_BATHY_50_floats_traj_',num2str(kk)],'-dpng')
    close all
    %%
    ...
    acc_plots
    hold off
    print('-r200',['FLOATS/AB_BATHY_50_floats_',num2str(kk)],'-dpng')
    close all
    %%
