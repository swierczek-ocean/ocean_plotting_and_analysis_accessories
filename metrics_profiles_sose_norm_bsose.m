function [] = metrics_profiles_sose_norm_bsose(varargin);
%% 
...
%   ABind = find(prof_lat>-60 & prof_lat<-30 & prof_lon>290 & prof_lon<350)
%   
%   prof_lat = prof_lat(ABind);
%   prof_lon = prof_lon(ABind);
%   prof_HHMMSS = prof_HHMMSS(ABind);
%   prof_descr = prof_descr(:,ABind);
%   prof_YYYYMMDD = prof_YYYYMMDD(ABind);
%   prof_date = prof_date(ABind);
%   depth = depth(ABind);
% 
%   prof_Tobs = prof_Tobs(ABind);
%   prof_Tweight = prof_Tweight(ABind);
%   prof_Sobs = prof_Sobs(ABind);
%   prof_Sweight = prof_Sweight(ABind);
%   prof_DICobs = prof_DICobs(ABind);
%   prof_DICweight = prof_DICweight(ABind);
%   prof_ALKobs = prof_ALKobs(ABind);
%   prof_ALKweight = prof_ALKweight(ABind);
%   prof_O2obs = prof_O2obs(ABind);
%   prof_O2weight = prof_O2weight(ABind);
%   prof_NO3obs = prof_NO3obs(ABind);
%   prof_NO3weight = prof_NO3weight(ABind);
%   prof_PO4obs = prof_PO4obs(ABind);
%   prof_PO4weight = prof_PO4weight(ABind);
%   prof_CHLobs = prof_CHLobs(ABind);
%   prof_CHLweight = prof_CHLweight(ABind);
%   prof_POCobs = prof_POCobs(ABind);
%   prof_POCweight = prof_POCweight(ABind);
%   prof_PHobs = prof_PHobs(ABind);
%   prof_PHweight = prof_PHweight(ABind);
%   prof_PCOobs = prof_PCOobs(ABind);
%   prof_PCOweight = prof_PCOweight(ABind);
%   prof_FEobs = prof_FEobs(ABind);
%   prof_FEweight = prof_FEweight(ABind);
