...
Obs_90_temp = [time,lat,lon+360,theta,salt];
Obs_90_temp = sortrows(Obs_90_temp,1);

start_date = 20161130;

start = find(Obs_90_temp(:,1)>start_date,1);

Obs_90_temp = Obs_90_temp(start:end,:);
...
for ii=1:396
    ...
    Obs_90(ii,:) = mean(Obs_90_temp(Obs_90_temp(:,1)==date_n,:),1);
    date = date+1;
end

clearvars -except Obs_90
